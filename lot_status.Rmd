...
main_only <- lots_and_closing %>%
  filter(lot_name == "Main") %>%
  mutate(weekday = as.factor(weekday),
         closed = as.factor(closed))

test_ts <- ts(main_only, start = c(2021, 1, 16), frequency = 7)
...
lot_model <- tslm(formula = closed ~ season, data = test_ts)
summary(lot_model)
checkresiduals(lot_model)
temp <- forecast(lot_model)
autoplot(temp)
